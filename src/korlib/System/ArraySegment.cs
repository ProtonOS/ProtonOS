...
namespace System
{
    ...
    public readonly struct ArraySegment<T> : IList<T>, IReadOnlyList<T>
    {
        ...
        /// <summary>Forms a slice out of the current ArraySegment starting at the specified index.</summary>
        ...
        public T[] ToArray()
        {
            if (_array == null || _count == 0)
                return new T[0];  // Avoid Array.Empty<T>() - not in AOT registry
        ...
        {
            // Inline hash computation to avoid issues with HashCode.Combine and object boxing
            int hash = 17;
            hash = hash * 31 + (_array?.GetHashCode() ?? 0);
            hash = hash * 31 + _offset;
            hash = hash * 31 + _count;
            return hash;
        }
