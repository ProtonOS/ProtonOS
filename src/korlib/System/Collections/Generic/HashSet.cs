...
public class HashSet<T> : ISet<T>, ICollection<T>, IReadOnlyCollection<T>
{
    ...
    public bool Remove(T item)
    {
        if (_buckets != null)
        {
            ...
            while (i >= 0)
            {
                // Note: Use array indexing instead of ref local to avoid JIT issues
    ...
    /// <summary>Attempts to get the value equal to the given value from the set.</summary>
    ...
    public struct Enumerator : IEnumerator<T>
    {
        ...
        public bool MoveNext()
        {
            ...
            // Note: Use direct array indexing instead of ref local to avoid JIT issues
            Entry[]? entries = _hashSet._entries;
            while ((uint)_index < (uint)_hashSet._count)
            {
                int idx = _index++;
                if (entries![idx].hashCode >= 0)
                {
                    _current = entries[idx].value;
                    return true;
                }
            }
